// from https://medium.com/geekculture/implementing-k-means-clustering-from-scratch-in-javascript-13d71fbcb31e
...
function kmeans(dataset_full, k, useNaiveSharding = true) {
  const dataset = dataset_full.map(item => [item[0], item[1]])
  if (dataset.length && dataset[0].length && dataset.length > k) {
    ...
    while (!shouldStop(oldCentroids, centroids, iterations)) {
      ...
      labels = getLabels(dataset_full, centroids);
