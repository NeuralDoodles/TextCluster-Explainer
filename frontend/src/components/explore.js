...
function Explore(data) {
  ...
  function handleAutoCluster(event) {
    appcontext.setClusterk(10) // should be adjustable
    // let position_vector = appcontext.plottedData.map(item => [item[0], item[1]])
    let clusters = kmeans(appcontext.plottedData, 10)["clusters"] // 10 should be clusterk, take care of when error is thrown
    appcontext.setClusters(clusters)
    console.log(clusters)
    console.log("clustercolors", appcontext.clustercolors)

    // console.log("kmeans", kmeansjs.kmeans())
    // let req = {
  ...
  if (appcontext.lassoed.length > 0) {
    ...
    appcontext.lassoed.forEach(element => {
      ...
      SelectedItemsInfo.push(element)
